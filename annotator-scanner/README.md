...
`Type Annotator Scanner` requires a context file to serialize the requested information at the designated directory. This context file should be written in `xml` format and the path to this file should be passed via the flag below:
`-XepOpt:AnnotatorScanner:ConfigPath=path_to_scanner.xml`


#### Config format
The context file is written in `xml` format has the structure below:
```xml
<scanner>
...
The context above will be generated by the `annotator-core` module automatically.
